...
func main() {
	...
	// var (
	// 	query      = flag.String("query", "noormahal", "Search term")
	// 	maxResults = flag.Int64("max-results", 10, "Max YouTube results")
	// )

    // Call the function from the "api" package
    // response, err := api.FetchYouTubeVideos(ctx, &query, &maxResults, developerKey)

    // if err != nil {
    //     fmt.Println("Failed to fetch YouTube videos:", err)
    // } else {
    //     fmt.Println("Fetched YouTube videos:", response)
    // }
